...
import { syncProfile } from './chromeProfileSync';

const RELAY_IMAGE = 'relay-agent';
const DEFAULT_CHROME_PROFILE = 'Profile 1'; // usc.edu
...
async function bootOne(): Promise<void> {
  ...
  // Sync Chrome profile for the warm container
  let chromeProfilePath: string | undefined;
  try {
    chromeProfilePath = await syncProfile(DEFAULT_CHROME_PROFILE, `warm-${ports._index}`);
    console.log(`[warmPool] Chrome profile synced for ${containerName}`);
  } catch (err) {
    console.warn(`[warmPool] Chrome profile sync failed: ${(err as Error).message}`);
  }
